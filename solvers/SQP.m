function [xfinal, costfinal, residual, info, options, stopreason] = SQP(problem0, x0, options)
    ...
    while true
        ...
        elseif options.verbosity >= 1
            if mod(iter, 100) == 0 && iter ~= 0
                fprintf('Iter: %d, Cost: %f, KKT resiidual: %.4e, Time: %.4e\n', iter, xCurCost, xCurResidual, ttime);
            end
        end
