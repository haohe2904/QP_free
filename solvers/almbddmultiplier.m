function [xfinal, info, residual, stopreason] = almbddmultiplier(problem0, x0, options)
    ...
    for OuterIter = 1 : options.maxOuterIter
        ...
        ttime = toc(Time);
        % #Verbosity modified
        if options.outerverbosity >= 3
            Inneriter = Inneriter + innerinfo(end).iter + 1;
            if OuterIter == 1
                fprintf('Iter: %d, Subiter: %d, KKT Residual: %.4e, Time: %.4e\n', OuterIter,Inneriter,xCurResidual, ttime)
                kkt_flag0 = floor(log10(xCurResidual));
            else
                kkt_flag1 = floor(log10(xCurResidual));
                if kkt_flag1 < kkt_flag0
                    fprintf('Iter: %d, Subiter: %d, KKT Residual: %.4e, Time: %.4e\n', OuterIter,Inneriter,xCurResidual, ttime)
                    kkt_flag0 = kkt_flag1;
                end
            end
        elseif options.outerverbosity >= 2
            fprintf('Iter: %d, KKT Residual: %.4e, Time: %.4e\n', OuterIter, xCurResidual, ttime)
        elseif options.outerverbosity == 1 && mod(OuterIter, 100) == 0 
            fprintf('Iter: %d, KKT Residual: %.16e, Time: %.4e\n', OuterIter, xCurResidual, ttime)
        end
        %
        
        % #Stopping criteria
        if xCurResidual < options.tolKKTres && tolgradnorm <= options.endingtolgradnorm  % #stopping criterion based on KKT_residual()
            %fprintf("KKT Residual and tolgradnorm tolerance reached\n")
            break;
        elseif dist == 0 && ~(updateflag_rho) && ~(updateflag_Lagmult) ...
            ...
            break; 
        elseif xCurResidual == Inf
            break;
        elseif xCurResidual < options.tolKKTres % #stopping criterion based on KKT_residual()
    ...
    function manvio = manifoldPowerViolation(xCur)
        ...
        manvio = 0;
        if contains(problem0.M.name(),'Product')
            y = xCur(:);
            manvio = abs(y.S'*y.S - 1)^2;
        elseif contains(problem0.M.name(),'Sphere')         
            y = xCur(:);
            manvio = abs(y.'*y - 1)^2;
