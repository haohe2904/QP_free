function [xfinal,costfinal,residual,KKTdata,dist,stopreason] = QP_free_rank2_mex_plot(m, n, k, P, A, eqindices, ineqindices, x0, options)
   ...
   % base = cell(dim,1);
    ...
    for iter = 1:maxiter
        ...
        if rho_flag == 0
            ...
            [qr_V, tau_V] = dgeqrf_mex([x.V, zerosV]);
            [qr_U, tau_U] = dgeqrf_mex([x.U, zerosU]);
            ...
            for i = 1:ineq_num
                ...
                ineqB = apply_q_mex(B0,qr_V, tau_V, 'T'); 
                %ineqB1 = fun_alpha(B0, vV, sV, n, k);
                ineqB = ineqB(k+1:n,:);
                %ineqB1 = ineqB1(k+1:n,:);
                ...
                C0 = zeros(m,k);  
                C0(row,:) = x.V(col,:);              
                ineqC = apply_q_mex(C0, qr_U, tau_U, 'T'); 
                ineqC = ineqC(k+1:m,:);
            ...
            for i = 1:eq_num
                ...
                B0 = zeros(n,k);  
                B0(col,:) = x.U(row,:);           
                eqB = apply_q_mex(B0,qr_V, tau_V, 'T'); 
                eqB = eqB(k+1:n,:);
        
                % Compute x.U_perp'*egrad c_eqs(x)*x.V
                C0 = zeros(m,k);  
                C0(row,:) = x.V(col,:);            
                eqC = apply_q_mex(C0,qr_U, tau_U, 'T'); 
                eqC = eqC(k+1:m,:);
            ...
            egradf = PXA;
            gradf = Proj_fixrank_mex(egradf, x, qr_U, tau_U, qr_V, tau_V, m, n, k);
            vgradf = [gradf.A(:); gradf.C(:); gradf.B(:)];
            egradLag = gradLagrangian(egradf, mus, lambdas, ineqindices, eqindices, ineq_num, eq_num, m, n);
            gradLag = Proj_fixrank_mex(egradLag, x, qr_U, tau_U, qr_V, tau_V, m, n, k);
        ...
        [HL_A, HL_B, HL_C] = HL_D2E_mex(P, egradLag, x, qr_U, tau_U, qr_V, tau_V, m, n, k, dim); 
       ...
       % HL = eye(dim);
        ...
        M1_main = apply_q_mex([zeros(k); K_main],qr_U, tau_U, 'N'); 
        M2_main = apply_q_mex([zeros(k); W_mian],qr_V, tau_V, 'N'); 
        main_dir.Up = M1_main;
        main_dir.Vp = M2_main;
        ...
        if KKT_res < cor_tol
            ...
            if norm(coeff,2) <= main_dir_norm
                ...
                M1_cor = apply_q_mex([zeros(k); K_cor],qr_U, tau_U, 'N');
                M2_cor = apply_q_mex([zeros(k); W_cor], qr_V, tau_V,'N'); 
                cor_dir.Up = M1_cor;
                cor_dir.Vp = M2_cor;
    ...
    xfinal = x; costfinal = xcost; residual = KKT_res; Xfinal = X;
    
            dist=zeros(1,maxiter);
            for i=1:maxiter
            dist(i) = norm(XI{i} - Xfinal, 'fro');
            end
