function [xfinal, costfinal, residual, stopreason] = QP_free_Oblique(d, s, A, C, X0, options)
   ...
   % M = obliquefactory(d,s);
    ...
    for iter = 1:maxiter
        ...
        for i = 1:ineq_num
            ...
            gradc0 = zerosd + X(row,col)*X(:,col);   % (I-x_ix_i^T)zerosd

            % Compute the vector representation of gradc by Algorithm 4 in Huang et al (2017).
            vgradc0 = gradc0 - 2*(gradc0'*Ob_v(:,col))*Ob_v(:,col);
            vgradc0(1) = [];
            vgradc = zerosdim;
        ...
        %egradf = egradf(:); vx = X(:);
        gradf = egradf - sum(egradf.*X,1).*X;
        vgradm = gradf - 2*sum(gradf.*Ob_v,1).*Ob_v;
        vgradm(1,:) = [];
        vgradf = vgradm(:);

        %%
        Mus = reshape(mus, [d,s]);
        rMus = Mus - sum(Mus.*X,1).*X;
        mMus = rMus - 2*sum(rMus.*Ob_v,1).*Ob_v;
        mMus(1,:) = [];
        vMus = mMus(:);
       ...
       % gradLagnorm=0;
        ...
        for i = 1:s
            ...
            for k = i:s
                if k == i
                    ...
                    BtH = H0 - 2*Ob_v(:,i)*(Ob_v(:,i)'*H0);
                    BtH(1,:) = [];
                    HtB = BtH';
                    BtHtB =  HtB - 2*Ob_v(:,i)*(Ob_v(:,i)'*HtB);
                    BtHtB(1,:) = [];
                    MH{(i-1)*s + k} = BtHtB;
                else
                    BtH = (2*Upsilon/s)*(I_d - 2*Ob_v(:,k)*Ob_v(:,k)');
                    BtH(1,:) = [];
                    HtB = BtH';
                    BtHtB =  HtB - 2*Ob_v(:,i)*(Ob_v(:,i)'*HtB);
                    BtHtB(1,:) = [];
        ...
        main_dir = [zeros(1,s);main_e_dirm] - 2*sum([zeros(1,s);main_e_dirm].*Ob_v,1).*Ob_v;
        ...
        if KKT_res < cor_tol
            ...
            if norm(coeff,2) <= main_dir_norm
                ...
                cor_dir = [zeros(1,s);cor_e_dir] - 2*sum([zeros(1,s);cor_e_dir].*Ob_v,1).*Ob_v;
            else
                cor_dir = 0;
            end
        end
    ...
    %% Support functions
    function [Ob_v, Ob_s] = Ob_fun_vs(X, d, s)
        Ob_v = zeros(d,s); Ob_s = zeros(1,s);
        for j = 1:s
            z = X(:,j);
            Ob_s(j) = -sign(z(1));
            z_ae1 = z; 
            z_ae1(1) = z(1) - Ob_s(j)*norm(z,2);
            Ob_v(:,j) = z_ae1/norm(z_ae1,2);
        end
    end
