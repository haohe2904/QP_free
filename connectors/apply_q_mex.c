...
void dormqr_(char *side, char *trans, mwSignedIndex *m, mwSignedIndex *n,
             mwSignedIndex *k, double *A, mwSignedIndex *lda, double *tau,
             double *C, mwSignedIndex *ldc, double *work, mwSignedIndex *lwork,
             mwSignedIndex *info);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    dormqr_(&side, &trans, &m, &n, &k, QR, &lda, TAU, C, &ldc, &work_opt, &lwork, &info);
    if (info != 0) {
        mxFree(C);
    ...
    /* 实际调用 dormqr */
    dormqr_(&side, &trans, &m, &n, &k, QR, &lda, TAU, C, &ldc, work, &lwork, &info);
    if (info != 0) {
        mxFree(C);
        mxFree(work);
