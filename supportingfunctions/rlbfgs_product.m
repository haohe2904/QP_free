function [x, cost, info, options] = rlbfgs_product(problem, x0, options)
...
% Change log: 
    ...
    if ~exist('x0', 'var')|| isempty(x0)
        xCur.a=rand(problem.dimR);
        xCur.b=rand(problem.dimR);
        xCur.S = M.rand(); 
        xCur.c=rand(1);
    else
        xCur = x0;
    ...
    xCurCost=problem.L_cost(xCur);
    xCurGradient=problem.L_grad(xCur);  
    xCurGradNorm = problem.pnorm(xCur, xCurGradient);
    ...
    while true
        ...
        [stepsize, xNext, newkey, lsstats] = ...
            linesearch_product(problem, xCur, p, xCurCost, ...
                            pinner(M, xCur, xCurGradient, p), ...
                            options, storedb, key);
        
        % Record the BFGS step-multiplier alpha which as effectively
        % selected. Toward convergence, we hope to see alpha = 1.
        alpha = stepsize/problem.pnorm(xCur, p);
        step.a=alpha*p.a;
        step.b=alpha*p.b;
        step.c=alpha*p.c;
        step.S=M.lincomb(xCur.S, alpha, p.S);
        
        
        % Query cost and gradient at the candidate new point.
        xNextCost=problem.L_cost(xNext);
        xNextGrad=problem.L_grad(xNext); 
        
        % Compute sk and yk
        sk.a=step.a;
        sk.b=step.b;
        sk.c=step.c;
        sk.S=M.transp(xCur.S, xNext.S, step.S);
        yk.a=xNextGrad.a-xCurGradient.a;
        yk.b=xNextGrad.b-xCurGradient.b;
        yk.c=xNextGrad.c-xCurGradient.c;
        yk.S=M.lincomb(xNext.S, 1, xNextGrad.S, ...
                             -1, M.transp(xCur.S, xNext.S, xCurGradient.S));

        % To make the Hessian operator more stable by normalizing the 
        % step taken.
        norm_sk = problem.pnorm(xNext, sk);
        sk.a=(1/norm_sk)*sk.a;
        sk.b=(1/norm_sk)*sk.b;
        sk.c=(1/norm_sk)*sk.c;
        sk.S=M.lincomb(xNext.S, 1/norm_sk, sk.S);
        yk.a=(1/norm_sk)*yk.a;
        yk.b=(1/norm_sk)*yk.b;
        yk.c=(1/norm_sk)*yk.c;
        yk.S=M.lincomb(xNext.S, 1/norm_sk, yk.S);
        
        inner_sk_yk = pinner(M, xNext, sk, yk);
        inner_sk_sk = problem.pnorm(xNext, sk)^2;    % ensures nonnegativity
        ...
        if inner_sk_sk ~= 0 && (inner_sk_yk / inner_sk_sk) >= cap
            ...
            scaleFactor = inner_sk_yk / problem.pnorm(xNext, yk)^2;
            ...
            if k >= options.memory
                ...
                for  i = 2 : options.memory
                    sHistory{i}.a=sHistory{i}.a;
                    sHistory{i}.b=sHistory{i}.b;
                    sHistory{i}.c=sHistory{i}.c;
                    sHistory{i}.S=M.transp(xCur.S, xNext.S, sHistory{i}.S);
                    yHistory{i}.a=yHistory{i}.a;
                    yHistory{i}.b=yHistory{i}.b;
                    yHistory{i}.c=yHistory{i}.c;
                    yHistory{i}.S=M.transp(xCur.S, xNext.S, yHistory{i}.S);
                end
                if options.memory > 1
        ...
        xCurGradNorm = problem.pnorm(xNext, xNextGrad);
        xCurCost = xNextCost;
    ...
    function [stepsize, newx, newkey, lsstats] = ...
            linesearch_product(problem, x, d, f0, df0, options, storedb, key)
        ...
        newx.b=x.b+t*d.b;
        newx.c=x.c+t*d.c;
        newx.S = problem.M.retr(x.S, d.S, t);
        newkey = storedb.getNewKey();
        newf = problem.L_cost(newx);
        cost_evaluations = 1;

       % Backtrack while the Armijo criterion is not satisfied.
       while ls_backtrack && newf > f0 + suff_decr*t*df0
        
           % Reduce the step size,
           t = contraction_factor * t;
       ...
       % As 
       % seen outside this function, stepsize is the size of the vector we
       % retract to make the step from x to newx. Since the step is alpha*d:
       norm_d = problem.pnorm(x, d);
       stepsize = t * norm_d;
...
function dir = getDirection(M, xCur, xCurGradient, sHistory, yHistory, ...
    ...
    for i = k : -1 : 1
        inner_s_q(1, i) = rhoHistory{i} * pinner(M, xCur, sHistory{i}, q);
        q.a=q.a-inner_s_q(1, i)*yHistory{i}.a;
        q.b=q.b-inner_s_q(1, i)*yHistory{i}.b;
        q.c=q.c-inner_s_q(1, i)*yHistory{i}.c;
        q.S=M.lincomb(xCur.S, 1, q.S, -inner_s_q(1, i), yHistory{i}.S);
    end
    r.a=scaleFactor*q.a;
    r.b=scaleFactor*q.b;
    r.c=scaleFactor*q.c;
    r.S = M.lincomb(xCur.S, scaleFactor, q.S);
    
    for i = 1 : k
         omega = rhoHistory{i} * pinner(M, xCur, yHistory{i}, r);
         r.a=r.a+(inner_s_q(1, i)-omega)*sHistory{i}.a;
         r.b=r.b+(inner_s_q(1, i)-omega)*sHistory{i}.b;
         r.a=r.a+(inner_s_q(1, i)-omega)*sHistory{i}.a;
         r.S=M.lincomb(xCur.S, 1, r.S, inner_s_q(1, i)-omega, sHistory{i}.S);
    end
    dir.a=-r.a;
    dir.b=-r.b;
    dir.c=-r.c;
    dir.S=M.lincomb(xCur.S, -1, r.S);
    
end

% Define the inner product on the product tangent vector space
function val=pinner(M, x, u, v)
val = u.a'*v.a+u.b'*v.b+M.inner(x.S, u.S, v.S) +u.c'*v.c;
end
